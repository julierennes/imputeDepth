...
library(VIM)
# Load data
banknotes.all <- read.table(file = "data//banknotes.dat", header = FALSE, sep = ",")
...
for (i in 1:k){
  ...
  if (i %% 100 < 1){
    save.image(paste("banknotes_one1-3_k", i, "_",
...
errors <- list(TukeyR2 = ems.depth.Tr2, zonoidM = ems.depth.zm,
               ...
               mean = ems.mean)
boxplot(errors, main = paste("Banknotes1-3, n = ", n, ", d = ", d, ", MCAR ",
                             pNA, ", k = ", i, sep = ""),
        names = c("Tur2", "zom",
                  "Mah", "MahR",
                  "em", "for", "knn", "pc1", "pc2",
